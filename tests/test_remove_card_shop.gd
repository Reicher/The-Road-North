...
func run() -> void:
	...
	_assert(_titles(grid) == ["Straight Road", "Straight Road", "Dead End", "Sleep", "Sleep"], "Expected every removable card copy to be shown")
	var dead_end_card := grid.get_child(2).get_node("Card") as CardView
	var sleep_card := grid.get_child(3).get_node("Card") as CardView
	...
	_assert(_titles(grid) == ["Straight Road", "Straight Road", "Dead End", "Sleep"], "Expected every remaining card copy in deck overview")
