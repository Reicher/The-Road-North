...
func run() -> void:
	...
	_assert(placement.get_sight() == 1, "Expected the player to start with Sight 1")
	inventory.add_item({"name": "Kikare", "effect": "+1 Sight", "sight_bonus": 1})
	_assert(placement.get_sight() == 2, "Expected Kikare in the inventory to grant +1 Sight")
	_assert(placement.is_in_sight(Vector2i(4, 2)), "Expected Kikare to allow a two-tile diagonal")
	_assert(not placement.is_in_sight(Vector2i(5, 4)), "Expected a three-tile horizontal offset to remain outside Sight 2")
	...
	_assert(placement.get_node("SightFog").get("fogged_positions").size() == 10, "Expected Kikare to expand the clear square to Sight 2")
