...
func _initialize() -> void:
	...
	_assert(event_types.has(DeckController.EVENT_CLEAR_PATH), "Expected event deck to include Clear Path")
	_assert(event_types.has(DeckController.EVENT_AMBUSH), "Expected event deck to include Ambush")
	_assert(event_types.has(DeckController.EVENT_WILD_BERRIES), "Expected event deck to include Wild Berries")
	_assert(event_types.has(DeckController.EVENT_LOST_BELONGINGS), "Expected event deck to include Lost Belongings")
