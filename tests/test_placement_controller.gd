...
func _initialize() -> void:
	...
	preview_tile.set_preview_encounter_data({"type": GameConstants.ENCOUNTER_CAMPFIRE})
	_assert(preview_visuals.get_node_or_null("EncounterPlaza") != null, "Expected an encounter preview to show its plaza")
	preview_tile.set_preview_encounter_data({})
	_assert(preview_visuals.get_node_or_null("EncounterPlaza") == null, "Expected a reused plain-road preview not to retain an encounter plaza")
	preview_tile.set_preview_encounter_data(placement.call("_get_preview_encounter_data"))
	...
	_assert(hand.cards.has(corner_card), "Expected dragging back into the hand to keep the card")
