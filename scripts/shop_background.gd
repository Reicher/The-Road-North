...
const BACKDROP_FILL := Color(0.105, 0.065, 0.035, 1.0)
const PLANK_A := Color(0.145, 0.085, 0.040, 1.0)
const PLANK_B := Color(0.105, 0.060, 0.030, 1.0)
const PLANK_SEAM := Color(0.035, 0.020, 0.012, 0.72)
const SHELF_BACK := Color(0.165, 0.095, 0.045, 0.96)
const SHELF_BACK_ALT := Color(0.120, 0.070, 0.035, 0.98)
const SHELF_EDGE := Color(0.245, 0.135, 0.055, 1.0)
const SHELF_DARK_EDGE := Color(0.055, 0.032, 0.018, 0.95)
const SHELF_HIGHLIGHT := Color(0.430, 0.255, 0.105, 0.62)
const BRASS := Color(0.720, 0.500, 0.205, 0.82)
...
func _draw_back_wall() -> void:
	...
	while x < size.x:
		...
		x += plank_width
		index += 1
	draw_rect(Rect2(Vector2.ZERO, size), Color(0.0, 0.0, 0.0, 0.22), false, 4.0)


func _draw_shelf(rect: Rect2, shelf: Control) -> void:
	var back := rect.grow_individual(-2.0, -4.0, -2.0, -SHELF_BOARD_HEIGHT)
	if back.size.y > 12.0:
		draw_rect(back, Color(0.055, 0.030, 0.015, 0.54))
	...
	draw_rect(board_shadow, Color(0.0, 0.0, 0.0, 0.36))
	_draw_board(board)
...
func _draw_board(board: Rect2) -> void:
	...
	while x < board.end.x - 8.0:
		draw_line(Vector2(x, board.position.y + 3.0), Vector2(x, board.end.y - 3.0), Color(0.040, 0.022, 0.010, 0.48), 1.0)
		x += 92.0


func _draw_post(post: Rect2) -> void:
	draw_rect(post, SHELF_DARK_EDGE)
	draw_rect(post.grow_individual(-3.0, 0.0, -3.0, 0.0), Color(0.150, 0.082, 0.035, 0.86))
...
func _draw_item_bays(shelf: Control) -> void:
	for node in shelf.find_children("*", "Control", true, false):
		...
		draw_rect(bay, Color(0.035, 0.020, 0.012, 0.48), false, 1.0)
...
func _draw_horizontal_segments(rect: Rect2) -> void:
	...
	while x < rect.end.x - 8.0:
		var seam_color := Color(0.040, 0.022, 0.010, 0.50 if index % 2 == 0 else 0.32)
...
func _draw_nails(rect: Rect2) -> void:
	...
	for point in positions:
		draw_circle(point + Vector2(1.0, 1.0), 4.0, Color(0.0, 0.0, 0.0, 0.35))
...
func _draw_grain_lines(rect: Rect2, offset: int) -> void:
	...
	while y < rect.end.y:
		...
		draw_line(Vector2(left, y), Vector2(right, y + sin(y * 0.05) * 2.0), Color(0.245, 0.150, 0.065, 0.22), 1.0)
		y += 42.0
